...
std::vector<double> PolynomialBasis::poly(const int id, const double value, const bool UVknot)
{
    ...
    {
        std::cout
            << "WRONG USAGE OF CLASS PolynomialBasis, YOU SHOULD INITIALIZE IT BY CALLING init()"
            << std::endl;
    }
...
void PartialBasis::init(Bsurface &surface)
{
    ...
    Ubasis = pb.Ubasis;
    Vbasis = pb.Vbasis;
    Ubasis_1 = do_partial(Ubasis);
    Vbasis_1 = do_partial(Vbasis);
    Ubasis_2 = do_partial(Ubasis_1);
    Vbasis_2 = do_partial(Vbasis_1);
    Uknot = surface.U;
    Vknot = surface.V;
    degree1 = surface.degree1;
    degree2 = surface.degree2;
...
double surface_energy_least_square(Bsurface &surface, const int i, const int j, PartialBasis &basis)
{
    ...
    if (coff_i < partial_i - degree1 || coff_i > partial_i + degree1)
        return 0;
    if (coff_j < partial_j - degree2 || coff_j > partial_j + degree2)
        return 0;

    // do partial Pij
    // for each block, (U_k1, U_(k1+1)), (V_k2, V_(k2+1)). the related control points are
    // k1-p,...,k1 and k2-q,..., k2
    double result = 0;
    for (int k1 = partial_i; k1 < partial_i + degree1 + 1; k1++)
    {
        for (int k2 = partial_j; k2 < partial_j + degree2 + 1; k2++)
        {
            ...
            // Suu part
            double value1 =
                construct_an_integration(degree1, surface.U, 2, 2, partial_i, coff_i, surface.U[k1],
                                         surface.U[k1 + 1], basis, false);
            double value2 = construct_an_integration(degree2, surface.V, 0, 0, partial_j, coff_j,
                                                     surface.V[k2], surface.V[k2 + 1], basis, true);
            // Suv part
            double value3 =
                construct_an_integration(degree1, surface.U, 1, 1, partial_i, coff_i, surface.U[k1],
                                         surface.U[k1 + 1], basis, false);
            double value4 = construct_an_integration(degree2, surface.V, 1, 1, partial_j, coff_j,
                                                     surface.V[k2], surface.V[k2 + 1], basis, true);
            // Svv part
            double value5 =
                construct_an_integration(degree1, surface.U, 0, 0, partial_i, coff_i, surface.U[k1],
                                         surface.U[k1 + 1], basis, false);
            double value6 = construct_an_integration(degree2, surface.V, 2, 2, partial_j, coff_j,
                                                     surface.V[k2], surface.V[k2 + 1], basis, true);
    ...
    Eigen::MatrixXd &energy_vv, Eigen::MatrixXd &energy_uv)
{
    ...
    for (int i = 0; i < uint; i++)
    {
        for (int j = 0; j < vint; j++)
        {
            ...
            // std::cout << "u0, u1, v0, v1 " << u0 << " " << u1 << " " << v0 << " " << v1 <<
            // std::endl;
            for (int k1 = 0; k1 < n_sample; k1++)
            {
                ...
                for (int k2 = 0; k2 < n_sample; k2++)
                {
                    ...
                    //  Suu
                    values_uu(k1, k2) = discrete_surface_partial_value_squared(
                        2, 0, Ni, Nj, surface, basis, uvalue, vvalue);
                    // Svv
                    values_vv(k1, k2) = discrete_surface_partial_value_squared(
                        0, 2, Ni, Nj, surface, basis, uvalue, vvalue);
                    // Suv
                    values_uv(k1, k2) = discrete_surface_partial_value_squared(
                        1, 1, Ni, Nj, surface, basis, uvalue, vvalue);
                }
            }
            double uusum = 0;
            double vvsum = 0;
            ...
            for (int k1 = 0; k1 < n_sample - 1; k1++)
            {
                ...
                {
                    uusum += (values_uu(k1, k2) + values_uu(k1, k2 + 1) + values_uu(k1 + 1, k2) +
                              values_uu(k1 + 1, k2 + 1)) /
                             4;
                    vvsum += (values_vv(k1, k2) + values_vv(k1, k2 + 1) + values_vv(k1 + 1, k2) +
                              values_vv(k1 + 1, k2 + 1)) /
                             4;
                    uvsum += (values_uv(k1, k2) + values_uv(k1, k2 + 1) + values_uv(k1 + 1, k2) +
                              values_uv(k1 + 1, k2 + 1)) /
                             4;
                }
            }
...
void Bsurface::detect_max_energy_interval(Bsurface &surface, const Eigen::MatrixXd &energy,
        ...
        for (int partial_j = 0; partial_j < surface.nv() + 1; partial_j++)
        {
            ...
            for (int coff_i = partial_i - degree1; coff_i <= partial_i + degree1; coff_i++)
            {
                ...
                for (int coff_j = partial_j - degree2; coff_j <= partial_j + degree2; coff_j++)
                {
                    ...
                    double value = surface_energy_least_square_tripletes(
                        surface, partial_i, partial_j, coff_i, coff_j, basis);
                                                        ...
                                                        PartialBasis &basis)
{
    ...
    // Compute total number of control points
    int psize = (surface.nu() + 1) * (surface.nv() + 1); // total number of control points.
    std::vector<Vector3d> cps(psize);                    // control points
    std::vector<Trip> tripletes;
    // SIDE STOP
    // Assemble left-hand side matrix components (energy + constraints)
    surface_least_square_lambda_multiplier_left_part(surface, paras, basis, tripletes);

    // Init and factor the sparse system matrix
    SparseMatrixXd matB;
    ...
    {
        Eigen::MatrixXd b =
            surface_least_square_lambda_multiplier_right_part(surface, paras, points, i);

        // Perform the solve to obtain control points and multipliers
        Eigen::MatrixXd p_lambda = solver.solve(b);
        if (solver.info() != Eigen::Success)
        {
            std::cout << "solving failed" << std::endl;
            return;
        ...
        // Extract and assign control points for this dimension
        push_p_lambda_vector_to_control_points(p_lambda, i, cps);
    }
    // Make control point list.
