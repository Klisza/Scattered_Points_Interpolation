...
class Bsurface
{
  public:
    ...
    int nv();
    void generate_interpolation_knot_vectors(int degree1, int degree2, std::vector<double> &Uknot,
                                             std::vector<double> &Vknot,
                                             const Eigen::MatrixXd &param_original,
                                             double &per_ours, const double per,
    ...
    void detect_max_energy_interval(Bsurface &surface, const Eigen::MatrixXd &energy,
    ...
    double max_interpolation_err(const Eigen::MatrixXd &ver, const Eigen::MatrixXd &param,
                                 Bsurface &surface);
    void surface_visulization(Bsurface &surface, const int nbr, Eigen::MatrixXd &v,
                              Eigen::MatrixXi &f);
    Vector3d BSplineSurfacePoint(const Bsurface &surface, const double upara, const double vpara);
    Vector3d BSplineSurfacePoint(const std::vector<std::vector<std::vector<double>>> &upolys,
                                 const std::vector<std::vector<std::vector<double>>> &vpolys,
                                 const std::vector<std::vector<std::vector<double>>> &tpolys,
                                 double u, double v, double t, const std::vector<double> &U,
                                 const std::vector<double> &V, const std::vector<double> &T,
...
template <typename Tp, typename valueT> class SurfaceOpt
{
  public:
...
class Bcurve
{
  public:
    ...
    int nu(); // nu + 1 is the number of control points in u direction
    bool curve_can_be_interpolated(const std::vector<double> &U, const int degree,
                                   const std::vector<double> &paras, int &prob_id);
    bool curve_can_be_interpolated(const std::vector<double> &U, const int degree,
                                   const Eigen::VectorXd &paras, int &prob_id);
    std::vector<double> fix_knot_vector_to_interpolate_curve(const int degree,
    ...
    void solve_control_points_for_fairing_curve(Bcurve &curve, const std::vector<double> &paras,
                                                const std::vector<Vector3d> &pts, const double a,
                                                const double b);
    // the output is the curve.control_points
    // trying to find a curve minimizing the energy, while interpolating the points whose parameters
    // are paras.
    void solve_control_points_for_fairing_curve(Bcurve &curve, const std::vector<double> &paras,
                                                const Eigen::MatrixXd &points, const double a,
                                                const double b);
...
class PolynomialBasis
{
  public:
    PolynomialBasis(
        Bsurface &surface); // get all the bases of the B-spline surface into polynomial format
    PolynomialBasis();
    void init(Bsurface &surface);
    std::vector<double> poly(const int id, const double value, const bool UVknot);
    void clear();
    // the (i,j) th element of the basis is the basis function defined on {U_i,U_{i+1}}, the j
...
class PartialBasis
{
  public:
    ...
    // do 1 order differential to a list (with different value as indices) of a list (with the id
    // 'i' in N_{i,p}(u)) of polynomials
    std::vector<std::vector<std::vector<double>>>
    do_partial(const std::vector<std::vector<std::vector<double>>> &basis);
};
// type converters
...
bool write_triangle_mesh(const std::string filename, const Eigen::MatrixXd &ver,
                         Eigen::MatrixXi &f);
