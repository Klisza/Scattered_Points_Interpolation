...
{
int Bcurve::nu() { return U.size() - 2 - degree; }
                               ...
                               const std::vector<Vector3d> &points)
{
    ...
    for (int i = 1; i < m; i++)
    { // from 1 to m-1
        ri[i] = points[i] - points[0] * Nip(0, degree, paras[i], U) -
                points[m] * Nip(n, degree, paras[i], U);
    }
...
bool Bcurve::curve_can_be_interpolated(const std::vector<double> &U, const int degree,
                                       const Eigen::VectorXd &paras, int &prob_id)
{
    prob_id = -1;
...
bool Bcurve::curve_can_be_interpolated(const std::vector<double> &U, const int degree,
                                       const std::vector<double> &paras, int &prob_id)
{
    Eigen::VectorXd prs;
...
std::vector<double> merge_two_knot_vectors(const std::vector<double> &U1,
                                           const std::vector<double> &U2, const int degree)
{
    std::vector<double> v;
...
// fix_nbr is a parameter that decide how many knot do we want to insert in this step. by default it
// is -1 which means the knot will be fully fixed. when fix_nbr>0, if insert more than fix_nbr
// knots, the function directly returns.
std::vector<double> fix_knot_vector_to_interpolate_curve_WKW(const int degree,
                                                             const std::vector<double> &init_vec,
                                                             const std::vector<double> &paras,
                                                             const double per, bool &fully_fixed,
                                                             const int fix_nbr)
{
    std::vector<double> result;
    ...
    if (hUsize == 2 * degree + 2)
    { // if there are too few parameters to fit, just return the initial one;
        fully_fixed = true;
        return init_vec;
    }
    std::vector<double> hU(hUsize);
    for (int i = 0; i < degree + 1; i++)
    { // from 0 to degree, the total number is degree+1
        hU[i] = 0;
        result.push_back(0);
    }
    for (int i = hUsize - degree - 1; i < hUsize; i++)
    { // from size-degree-1 to size-1, degree + 1 elements
        hU[i] = init_vec.back();
    }
    ...
    { // from degree+1 to s
        ...
        double belta =
            ((paras[i] + paras[i - 1]) / 2 - paras[i - degree]) / (paras[i] - paras[i - degree]);
        ...
        while (init_vec[i + interval] < a)
        {
            ...
            if (i + interval > m)
            { // when i + interval = m+1, it will be 1
                breakloop = true;
    ...
    {
        // fully_fixed = true;
        assert(result.size() == hU.size()); // by default, the two sizes should be the same. but if
                                            // not fully fixed, this is not true
    }
...
std::vector<double>
Bcurve::fix_knot_vector_to_interpolate_curve(const int degree, const std::vector<double> &init_vec,
                                             const std::vector<double> &paras, const double per,
                                             bool &fully_fixed, const int fix_nbr)
{
    return fix_knot_vector_to_interpolate_curve_WKW(degree, init_vec, paras, per, fully_fixed,
                                                    fix_nbr);
}
// TODO add a parameter weight to control the percentage
// per in [0,1]. when per =0, we have larger tolerance for feasible points, which means it converges
// to tranditional methods
                                                           ...
                                                           const int degree, const double per)
{
    ...
    if (para != U.back())
    { // when para !=1, we check which interval it is in
    ...
    { // if para == 1, the nth control point is feasible
        result.resize(1);
        result[0] = nu;
        return result;
    }
    ...
    double alpha_pre =
        (U[which + degree] - (U[which] + U[which + 1]) / 2) / (U[which + degree] - U[which]);
    double belta_pre = ((U[which + 1] + U[which]) / 2 - U[which - degree + 1]) /
                       (U[which + 1] - U[which - degree + 1]);
    ...
    std::string compare;
    if (para <= b)
    { // if para not too close to U[i+1], then N(i-p) is feasible
    ...
    else
    {
        ...
        // std::cout << para << compare<<" is too close to " << U[which + 1]<<"i-p is
        // "<<which-degree<<" N(i-p) is "<<Nip(which-degree,degree,para,U) << std::endl;
    }
    for (int k = which - degree + 1; k < which; k++)
    { // from i-p to i-1
        result.push_back(k);
    }
    if (para >= a)
    { // if para not too close to U[i], then N(i) is feasible
    ...
    else
    {
        ...
        // std::cout << para <<compare<< " is too close to " << U[which] << "i is " << which  << "
        // N(i) is " << Nip(which, degree, para, U) << std::endl;
    }
