...
{
// Returns the highest valid control-point index "n" in the u-direction.
int Bsurface::nu() { return U.size() - 2 - degree1; }
// Returns the highest valid control-point index "n" in the v-direction.
int Bsurface::nv() { return V.size() - 2 - degree2; }

// TODO make this function more efficient by removing 0 valued elements
Vector3d BSplineSurfacePoint_(const int degree1, const int degree2, const std::vector<double> &U,
                              const std::vector<double> &V, const double upara, const double vpara,
                              const std::vector<std::vector<Vector3d>> &control)
{
...
//
Vector3d Bsurface::BSplineSurfacePoint(const Bsurface &surface, const double upara,
                                       const double vpara)
{
    return BSplineSurfacePoint_(surface.degree1, surface.degree2, surface.U, surface.V, upara,
                                vpara, surface.control_points);
}
Vector3d Bsurface::BSplineSurfacePoint(const std::vector<std::vector<std::vector<double>>> &upolys,
                                       const std::vector<std::vector<std::vector<double>>> &vpolys,
                                       const std::vector<std::vector<std::vector<double>>> &tpolys,
                                       double u, double v, double t, const std::vector<double> &UKV,
                                       const std::vector<double> &VKV,
                                       const std::vector<double> &TKV,
                                       const std::vector<std::vector<std::vector<Vector3d>>> &CPs,
                                       const int udegree, const int vdegree, const int tdegree)
{
...
// id_list may contain id, if contains, return the one right after id; if the one is the id is the
// last of id_list, still return id. this happens when:
/*
t0: oxoo
t1: ooxx
t2:     xooo
t3:     oxoo
*/
// where the xs are the choosen ones. x in the first row is the choosen control point of t0, and
// then t1 can choose two control points
...
// get feasible control point matrix. if checking v direction control points (v_direction=true),
// make sure that the Uin knot vector is already fixed. for the explanation of Ugrid, Vgrid and
// UVmap, see function generate_UV_grid() in 'mesh_processing.h'
Eigen::MatrixXi get_feasible_control_point_matrix(
    const int degree1, const int degree2, const std::vector<double> &Uin,
    const std::vector<double> &Vin, const bool v_direction, const std::vector<double> &Ugrid,
    const std::vector<double> &Vgrid, const Eigen::MatrixXi &UVmap, const int nbr_para,
    std::vector<std::vector<std::array<int, 2>>> &para_to_feasible, const double per,
    per_too_large &per_flag, std::vector<int> &feasible_order)
{
    ...
    Eigen::MatrixXi result =
        Eigen::MatrixXi::Constant(rows, csize, -1); // initialize the matrix with -1
    ...
    // get basic feasible points, there are overlaps meaning this control points affect more than
    // one parameter
    for (int i = 0; i < rows; i++)
    { // for each iso-line
        for (int j = 0; j < other_grid.size(); j++)
        {
            ...
            std::vector<int> basic_fp =
                feasible_control_point_of_given_parameter(para, kv, degree, per);
    ...
    // next deal with duplication to generate a reduced_feasible matrix, for each ith row and jth
    // column, there are at most 1 element
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        for (int j = 0; j < csize; j++)
        {
            ...
            if (ij_size == 1)
            {                                     // if no overlape, directly select this one
...
Eigen::MatrixXi select_FCP_based_on_weight_naive(
    const Eigen::MatrixXi &fcp, std::vector<std::vector<std::array<int, 2>>> &para_to_feasible,
    ...
    for (int i = 0; i < para_to_feasible.size(); i++)
    {
        ...
        else
        {
            ...
            for (int j = 0; j < highest.size(); j++)
            {
                ...
                int id1 = highest[j][1];
                result(id0, id1) = i;
                if (weight_matrix(id0, id1) == 1)
                { // if no one shares fcp, meaning the parameter is 0 or 1, choose one fcp is enough
                    break;
                }
            }
        }
...
bool fcp_weight_validation(const Eigen::MatrixXi &fcp, const Eigen::MatrixXd &weight_matrix)
{
    int row = fcp.rows();
...
// maximal_selection is the maximal number of fcps we select in this step
Eigen::MatrixXi
select_FCP_based_on_weight(const Eigen::MatrixXi &fcp,
                           std::vector<std::vector<std::array<int, 2>>> &para_to_feasible,
                           const Eigen::MatrixXd &weight_matrix, bool &updated,
                           const int maximal_selection, const std::vector<int> &feasible_order)
{
    ...
    for (int i = 0; i < para_to_feasible.size(); i++)
    {
        ...
        if (selected[i].empty())
        {
            ...
            for (int j = 0; j < para_to_feasible[i].size(); j++)
            {
                int id0 = para_to_feasible[i][j][0];
                int id1 = para_to_feasible[i][j][1];
                std::cout << para_to_feasible[i][j][0] << " " << para_to_feasible[i][j][1]
                          << std::endl;
                double w = weight_matrix(id0, id1);
    ...
    if (updated == false)
    { // the weight matrix will not renew, now force to remove some redundant fcps,
        for (int i = 0; i < feasible_order.size(); i++)
        {
            int which = feasible_order[i];
            if (para_to_feasible[which].size() > 1)
            { // this is a redundant fcp
                selected_nbr += 1;
...
// pick one fcp for each parameter. this function is used only when the selecting based on weight is
// finished but still have redundant fcps
Eigen::MatrixXi remove_redundant_FCP(const Eigen::MatrixXi &fcp,
                                     std::vector<std::vector<std::array<int, 2>>> &para_to_feasible)
{
    int nbr_re = 0;
    Eigen::MatrixXi result = Eigen::MatrixXi::Constant(fcp.rows(), fcp.cols(), -1);
...
Eigen::MatrixXd weight_matrix_calculation(const Eigen::MatrixXi &fcp,
                                          const Eigen::MatrixXi &interval_matrix, const int degree)
{
    ...
    { // for each column
        for (int i = 0; i < rows; i++)
        {
            ...
            std::vector<int> interval_info(
                degree + 2,
                0); // h = interval_info[r] means there are h fcps sharing r control points
            if (interval < 0)
            { // interval value is -2: no other fcp share control points with it
                weight_matrix(i, j) = 1;
            }
            else
            {                        // search this column if there is overlap
                int col_nbr_pts = 0; // how many points there are in this column (except for whose
                                     // parameter is 0 or 1)
                ...
                {
                    if (fcp(k, j) < 0)
                    { // if this point does not exist
                        continue;
                    }
                    int other_interval = interval_matrix(k, j);
                    if (other_interval < 0)
                    { // if -1 or -2, no need to count
                        continue;
                    }
                    col_nbr_pts += 1;
                    ...
                    int nbr_share =
                        degree + 1 -
                        diff; // meaning that fcp(i,j) share nbr_share points with fcp(k,j).
                    if (k < i)
                    {
                        col_before_nbr += 1;
                    }
                ...
                double weight =
#ifndef WEIGHT_NAIVE
                    double(row_nbr_pts) * 1 / double(col_before_nbr) *
#endif
...
// TODO
Eigen::MatrixXi calculate_active_control_points_from_feasible_control_points(
    const Eigen::MatrixXi &fcp, const bool v_direction, const std::vector<double> &Uknot,
    const std::vector<double> &Vknot, const Eigen::MatrixXd &paras, const int degree1,
    const int degree2, std::vector<std::vector<std::array<int, 2>>> &para_to_feasible,
    const int target_steps, const std::vector<int> &feasible_order)
{
    assert(paras_to_feasible_and_fcp_validation(para_to_feasible, fcp));
    ...
    int degree;
    if (v_direction)
    { // if control points belongs to iso-v lines, then we are considering v knot vector
        kv = Vknot;
        degree = degree2;
    }
    else
    ...
    // now in interval_matrix, value = -1 means no fcp here; value = -2 means the parameter is 0 or
    // 1; otherwise, the value implies the parameter is in [u_value, u_(value+1)] interval_matrix no
    // need updating, fcp need to be updated
    ...
    while (updated)
    {
        ...
        // initialize interval matrix. it will record which interval [u_i, u_(i+1)] does the
        // parameters in
        ...
        selected_fcp = select_FCP_based_on_weight(selected_fcp, para_to_feasible, weight_matrix,
                                                  updated, maximal_processing, feasible_order);
...
std::vector<double> get_iso_line_parameters_from_ACP(const Eigen::MatrixXi &ACP, const int id,
...
bool progressively_generate_interpolation_knot_vectors(
    const bool v_direction, int degree1, int degree2, std::vector<double> &Uknot,
    std::vector<double> &Vknot, const std::vector<double> Ugrid, const std::vector<double> &Vgrid,
    const Eigen::MatrixXi &grid_map, const Eigen::MatrixXd &param, const double per_ours,
    const double per, const int target_steps, const bool enable_max_fix_nbr,
    per_too_large &per_flag)
{
    ...
    Eigen::MatrixXi FCP = get_feasible_control_point_matrix(
        3, 3, Uknot, Vknot, v_direction, Ugrid, Vgrid, grid_map, nbr_para, para_to_feasible,
        per_ours, per_flag, feasible_order);
...
#else
        calculate_active_control_points_from_feasible_control_points(
            FCP, v_direction, Uknot, Vknot, param, degree1, degree2, para_to_feasible, target_steps,
            feasible_order);
#endif
#endif
    ...
    if (v_direction)
    { // if checking iso-v lines, then we are fixing V knot vector
        kv = Vknot;
        kv_other = Uknot;
    }
    else
    ...
    if (enable_max_fix_nbr)
    {
        ...
        nbr_fixed_in_each_itr = std::max(
            1, std::abs(int(
                   kv.size() -
                   kv_other.size()))); // std::max((int)sqrt(estimate_nbr_cp) / target_steps, 2);
    ...
    for (int i = 0; i < ACP.cols(); i++)
    {
        ...
        std::vector<double> parameters =
            get_iso_line_parameters_from_ACP(ACP, i, param, v_direction);
        std::vector<double> kv_new = fix_knot_vector_to_interpolate_curve_WKW(
            3, kv, parameters, per, fully_fixed, nbr_fixed_in_each_itr);
        ...
        if (enable_max_fix_nbr)
        {
            if (kv.size() > kv_other.size())
            { // the size difference between Uknot and Vknot decides which knot vector to refine
                break;
            }
...
std::vector<double> get_iso_line_parameters(const int degree1, const int degree2,
                                            const bool v_direction, const int line_id,
                                            const std::vector<double> &Ugrid,
                                            const std::vector<double> &Vgrid,
                                            const Eigen::MatrixXi &grid_map)
{
    std::vector<double> result;
...
std::vector<double>
update_knot_vector_based_on_grid(const int degree1, const int degree2, const bool v_direction,
                                 const std::vector<double> &Ugrid, const std::vector<double> &Vgrid,
                                 const Eigen::MatrixXi &grid_map, const double per,
                                 const std::vector<double> &Uin)
{
    ...
    for (int i = 0; i < size; i++)
    {
        std::vector<double> paras =
            get_iso_line_parameters(degree1, degree2, v_direction, i, Ugrid, Vgrid, grid_map);
        bool fully_fixed = false;
...
void Bsurface::generate_interpolation_knot_vectors(
    int degree1, int degree2, std::vector<double> &Uknot, std::vector<double> &Vknot,
    const Eigen::MatrixXd &param_original, const double per_ours, const double per,
    const int target_steps, const bool enable_max_fix_nbr, per_too_large &per_flag)
{
    ...
    // initialize U
    Utemp = update_knot_vector_based_on_grid(degree1, degree2, true, Ugrid, Vgrid, grid_map, per,
                                             Utemp);
    std::cout << "finished initialize Uknot" << std::endl;

    // initialize V
    Vtemp = update_knot_vector_based_on_grid(degree1, degree2, false, Ugrid, Vgrid, grid_map, per,
                                             Vtemp);
    ...
    while (!finished)
    {
        finished = progressively_generate_interpolation_knot_vectors(
            v_direction, degree1, degree2, Utemp, Vtemp, Ugrid, Vgrid, grid_map, param_original,
            per_ours, per, target_steps, enable_max_fix_nbr, per_flag);
        if (per_flag.flag == false)
        {
            return;
        }
    ...
    if (post_processing)
    {
        ...
        Utemp = Uknot;
        finished = progressively_generate_interpolation_knot_vectors(
            true, degree1, degree2, Utemp, Vtemp, Ugrid, Vgrid, grid_map, param_original, per_ours,
            per, target_steps, false, // not enable_max_fix_nbr
            per_flag);
        if (per_flag.flag == false)
        {
        ...
        Vtemp = Vknot;
        finished = progressively_generate_interpolation_knot_vectors(
            false, degree1, degree2, Utemp, Vtemp, Ugrid, Vgrid, grid_map, param_original, per_ours,
            per, target_steps, false, // not enable_max_fix_nbr
            per_flag);
        if (per_flag.flag == false)
        {
        ...
        std::cout << "post processing results, " << Utemp.size() << " " << Vtemp.size()
                  << std::endl;
        print_vector(Utemp);
        print_vector(Vtemp);
    }
...
void Bsurface::generate_interpolation_knot_vectors(
    int degree1, int degree2, std::vector<double> &Uknot, std::vector<double> &Vknot,
    const Eigen::MatrixXd &param_original, double &per_ours, const double per,
    const int target_steps, const bool enable_max_fix_nbr)
{
    per_too_large per_flag;
    per_flag.flag = false;
    ...
    while (1)
    {
        generate_interpolation_knot_vectors(degree1, degree2, Uknot, Vknot, param_original,
                                            per_ours_tmp, per, target_steps, enable_max_fix_nbr,
                                            per_flag);
        if (per_flag.flag)
        {
            break;
...
double Bsurface::max_interpolation_err(const Eigen::MatrixXd &ver, const Eigen::MatrixXd &param,
                                       Bsurface &surface)
{
    double err = 0;
...
void B_spline_surface_to_mesh(Bsurface &surface, const int pnbr, Eigen::MatrixXd &ver,
...
void Bsurface::surface_visulization(Bsurface &surface, const int nbr, Eigen::MatrixXd &v,
                                    Eigen::MatrixXi &f)
{
