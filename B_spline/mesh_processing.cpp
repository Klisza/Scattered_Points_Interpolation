...
void map_vertices_to_square(const Eigen::MatrixXd &V, const Eigen::VectorXi &bnd,
                            Eigen::MatrixXd &bnd_uv)
{
    ...
    pdis[pdis.size() - 1] =
        pdis[pdis.size() - 2] + (V.row(bnd[0]) - V.row(bnd[bnd.size() - 1])).norm();
    ...
    {
        bnd_uv.row(i + 1) << (pdis[i] - pdis[corners[1] - 1]) /
                                 (pdis[corners[2] - 1] - pdis[corners[1] - 1]),
            1;
    }
    ...
    {
        bnd_uv.row(i + 1) << 1,
            1 - (pdis[i] - pdis[corners[2] - 1]) / (pdis[corners[3] - 1] - pdis[corners[2] - 1]);
    }

    // v=0, u decrease
    for (int i = corners[3]; i < pdis.size() - 1; i++)
    {
        bnd_uv.row(i + 1) << 1 - (pdis[i] - pdis[corners[3] - 1]) /
                                     (pdis[pdis.size() - 1] - pdis[corners[3] - 1]),
            0;
    }
...
// this is to remove some faces whose x, y or z coordinates (the parameter 'axis') of vertices
// larger than 'value' axis select from 0, 1, 2.
...
void mesh_parameterization(const std::string &meshfile, Eigen::MatrixXd &V, Eigen::MatrixXd &param,
                           Eigen::MatrixXi &F)
{
...
double shortest_edge_length_of_parametric_domain(const Eigen::MatrixXd &paras,
...
void constrained_delaunay_triangulation(const Eigen::MatrixXd &V, const Eigen::VectorXi &Edge_ids,
                                        Eigen::MatrixXi &F)
{
...
// given parameters and the connectivity, get the U and V parameters, and a map showing the
// positions of the points in U and V
void generate_UV_grid(const Eigen::MatrixXd &param, std::vector<double> &U, std::vector<double> &V,
                      Eigen::MatrixXi &map)
{
    ...
    std::priority_queue<std::pair<double, int>, std::vector<std::pair<double, int>>, decltype(cmp)>
        queue_u(cmp);
    std::priority_queue<std::pair<double, int>, std::vector<std::pair<double, int>>, decltype(cmp)>
        queue_v(cmp);
...
bool is_point_in_triangle_2d(const Vector2d &v0, const Vector2d &v1, const Vector2d &v2,
                             const Vector2d &p, int &orientation, int &statement)
{
...
// 3, on edge F2, F0
// since in our algorithm, the point is not possible on the vertices of the mesh
int find_one_ring_for_param(const double u, const double v, const Eigen::MatrixXd &param,
                            const Eigen::MatrixXi &F, int &orientation, int &statement)
{
    Vector2d p(u, v);
    for (int i = 0; i < F.rows(); i++)
    {
...
inline int orient2d(const Eigen::Vector2d &p, const Eigen::Vector2d &q, const Eigen::Vector2d &r)
{
    igl::predicates::exactinit();
    return (int)igl::predicates::orient2d(p, q, r);
}

inline bool point_on_segment2d(const Eigen::Vector2d &p, const Eigen::Vector2d &a,
                               const Eigen::Vector2d &b, bool know_colinear)
{
    if (!know_colinear)
    {
        if (orient2d(p, a, b) != 0)
...
// 0: no intersectin, 1: intersect interior. 2. intersect q0 or q1. 3. the point p0 is on the
// segment the code is only for the purpose of our code, not exactly solving the generic problems
...
// 0: not intersect
// 1: intersect
int ray_source_in_loop(const Eigen::Vector3d &p0in, const Eigen::Vector3d &p1in,
...
void classifyOriginalQuads(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                           const std::vector<int> &pIn, std::vector<int> &quadFull,
                           std::vector<int> &quadCut, std::vector<std::array<bool, 4>> &fvin,
                           std::vector<bool> &flags)
{
    ...
    for (int i = 0; i < fnbr; i++)
    {
        ...
        if (counter > 0 && counter < 4)
        {
            ...
            // if(i==304)
            // {
            //     std::cout << "checking " << i << ", the counter, " << counter << " verin, " <<
...
void cleanFaceList(const std::vector<int> &map, const std::vector<int> &quadFull,
                   const Eigen::MatrixXi &F, std::vector<std::array<int, 4>> &Fclean)
{
    Fclean.resize(quadFull.size());
    for (int i = 0; i < quadFull.size(); i++)
    {
        Eigen::Vector4i qd = F.row(quadFull[i]);
        Fclean[i] = {map[qd[0]], map[qd[1]], map[qd[2]], map[qd[3]]};
        if (map[qd[0]] < 0 || map[qd[1]] < 0 || map[qd[2]] < 0 || map[qd[3]] < 0)
        {
...
template <typename T>
void cleanFaceList(const std::vector<int> &map, std::vector<T> &faces,
...
void writeObjLoop(const std::string filename, const Eigen::MatrixXd &V,
                  const std::vector<int> &vmap, const std::vector<int> &Loop)
{
    std::ofstream fout;
    fout.open(filename);
    for (int i = 0; i < V.rows(); i++)
                                            ...
                                            const Eigen::MatrixXd &Vl, bool &found, int &round)
{
    ...
    for (int i = 0; i < vnbr; i++)
    {
        ...
        if (check)
        {
            ...
            if (!intersect)
            {
                std::cout << "numerical problem happens, we better implement this function by "
                             "ourselves\n";
                exit(0);
            }
            if (t0 < 1e-6)
                          ...
                          std::vector<Eigen::Vector3d> &intersections)
{
    for (CGMesh::EdgeIter e_it = mesh.edges_begin(); e_it != mesh.edges_end(); ++e_it)
    {
        ...
        Eigen::Vector3d intersection =
            edge_loop_intersectionPoint(p0, p1, V.row(vid0), V.row(vid1), Vl, found, round);
        if (found)
        {
            ehs.push_back(eh);
...
int edgeCutted(const int vf, const int vt, const Eigen::Vector4i &quad, CGMesh &mesh,
               const std::vector<CGMesh::EdgeHandle> &ehs, const std::array<bool, 4> &fvin)
{
    int result = -1;
...
void splitIntersectedQuads(CGMesh &mesh, const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                           ...
                           const std::vector<int> &quadCut,
                           const std::vector<std::array<bool, 4>> &fvin,
                           const std::vector<bool> &Vflags, std::vector<std::array<int, 4>> &rQuads,
                           std::vector<std::array<int, 3>> &rTris,
                           std::vector<std::array<int, 5>> &rPenta, std::vector<int> &bLoop)
{
    int vnbr = V.rows();
    ...
    for (int i = 0; i < quadCut.size(); i++)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            if (notin)
            {
                ...
                for (CGMesh::FaceVertexIter fvitr = mesh.fv_begin(fh); fvitr != mesh.fv_end(fh);
                     ++fvitr)
                {
        ...
        if (vloop.size() != 6)
        {
            ...
            exit(0);
            continue; // TODO This may skip some intersected shapes on the boundary, but resolution
                      // wise it is fine.
        }
...
int whichHalfedge(const int v0, const int v1, CGMesh &mesh,
                  const std::vector<CGMesh::EdgeHandle> &ehs)
{
...
void findConnectivityForInnerVertices(const std::vector<bool> &Vflags, const std::vector<int> &pin,
                                      const int vnbr, CGMesh &mesh,
                                      const std::vector<CGMesh::EdgeHandle> &ehs,
                                      std::vector<std::array<int, 5>> &connect)
{
    for (int i = 0; i < pin.size(); i++)
    {
        ...
        for (CGMesh::VertexOHalfedgeIter voh_it = mesh.voh_begin(vh); voh_it != mesh.voh_end(vh);
             ++voh_it)
        {
            ...
            if (Vflags[idout]) // this vertex is inside the boundary loop, add it into the
                               // neighbours
            {
                nbs.push_back(idout);
            }
...
void cutBoundaryGenerateTopology(const Eigen::MatrixXd &Vquad, const Eigen::MatrixXi &Fquad,
                                 const Eigen::MatrixXd &Vcurve, Eigen::MatrixXd &Vout,
                                 Eigen::MatrixXi &Fout)
{
    int vnbr = Vquad.rows();
    int fnbr = Fquad.rows();
    ...
    std::cout << "there are " << quadFull.size() << " quads are full, and " << quadCut.size()
              << " are cutted\n";
    CGMesh mesh;
    ...
    splitIntersectedQuads(mesh, Vquad, Fquad, Vcurve, ehs, quadCut, fvin, vFlags, rQ, rT, rP,
                          bLoop);
    std::cout << "new generated quads, " << rQ.size() << ", trians, " << rT.size() << ", pentas, "
              << rP.size() << "\n";
    std::vector<std::array<int, 5>> connect;
    ...
    auto extendPolygon =
        []<typename T>(std::vector<T> &inshape, std::vector<std::vector<int>> &extended)
    {
